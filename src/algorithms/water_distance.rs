    ...
    while let Some(fid) = queue.pop() {
        ...
        for (neighbor_fid,_) in &tile.neighbors {
            ...
            if is_land && (neighbor.terrain.is_ocean() || neighbor.lake_id.is_some()) {
                ...
                water_count += 1;
            } else if !is_land && !neighbor.terrain.is_ocean() && neighbor.lake_id.is_none() {
                shore_distance.get_or_insert_with(|| -1);
            }
        }
