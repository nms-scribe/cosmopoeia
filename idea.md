...
# Algorithms To Work Out:

## Curving Borders:
TODO: I think this all looks nice.

### Bezierify Coastline

TODO: Note that, if my grouping_id didn't have separate items for lakes and lake-islands, I could add that to the written shapes -- for that matter, I could do this in convert-heightmap and assign the id of the created polygons to a field in tiles. Grouping_id could be just that, and would not be different for lakes.

* tile_iterator = get an iterator tiles, filtered so that only non-ocean tiles are included (lakes are fine)
* if tile = iterator.next:
  * new_geometry = tile.geometry.clone()
  * while tile = iterator.next:
    * new_geometry = new_geometry.union(tile.geometry)
* ocean = get a rectangle polygon covering the whole extent of the thingie, with points along the border at 1 unit intervals.
* if new_geometry is multipolygon:
  * for geometry in new_geometry.geometries
    * bezierify
    * write a new feature to "coastline" layer with the bezierified polygons
    * ocean = ocean.difference(polygon)
* else:
  * bezeierify the one thingie
  * write the feature to the "coastline" layer with the bezierified polygons
  * ocean = ocean.difference(polygon)
* write the ocean to an "ocean" layer (split into multiple polygons?)
* save
...
* for tile in tile_layer
  * key,tile_geometry = if <field> is some:
    * (field,tile.geometry.clone())
  * else if shore_distance == -1:
    * valid_neighbors = map -- might need something a little better so I can index it by number
    * for neighbor_id in neighbors:
      * if neighbor.field is some:
        * valid_neighbors[neighbor.field].push(neighbor_id)
    * chosen = if valid_neighbors.len > 1:
      * pick the neighbor with the most choices, if it's a tie, pick based off of the lowest value fraction of x or something like that.
      * chosen is the key for that one
    * else
      * chosen is the first and only key in the table
    * (chosen,tile.geometry.clone)
  * let new_geometry = new_polygon_map.get(key)
      * value.geometry.union(tile.geometry)
    * else:
      * tile_geometry
  * new_polygon_map.insert(key,new_geometry)
* for (key,value) in polygon_map:
...
* For each polygon in the layer:
  * for each vertex in all rings:
    * vertex_counts[vertex].0 += 1
* segment_cache = some sort of collection where it is easy to store segments and find out if they're duplicated, and then get a unique id back. I have to be able to look for segments reversed as well, and return the same id, but provide an indicator that it's reversed. -- I could possibly use a QuadTree for this, combined with a list for storing the things. I only need to index from the beginning and the end points, and store multiple segments. Then, when I want to find if a segment is cached, I just need to compare if there are multiple segments stored in the cache. -- Alternatively, I only need to index from one point. When inserting, that's the start point, when checking I'll be checking with the end point.
* polygon_segments = map
-- split into line segments
* for each multi_polygon in the layer: 
  * polygons = list
  * for each polygon in multi_polygon:
    * rings = list
    * for each ring:
      * segments = list
      * current_segment = vec[]
      * vertices = iterator of vertexes in ring
      * if vertex = vertices.next()
        * prev_vertex_count = vertex_index[vertex]
        * current_segment.push(vertex)
        * for each vertex in remaining vertices:
          * next_vertex = vertex_index[vertex]
          * if next_vertex_count == 2 && prev_vertex_count == 1 || -- was non-shared, now shared by two polygons
               * next_vertex_count == 1 && prev_vertex_count == 2 || -- was shared by two polygons, now not shared
               * next_vertex_count > 2 -- this is a border intersection with more than two polygons, but only two can share it at a time, so we only split it once
            * current_segment.push(vertex); -- end the last one
            * segments.push(current_segment)
            * current_segment = vec[vertex]
          * else:
            * current_segment.push(vertex)
          * prev_vertex = next_vertex
        * segments.push(current_segment) -- push the last one onto the segments
        * if segments.len > 1: -- join the first and last segments
          * if (Some(first),Some(last)) = (segments.remove(0),segments.remove_last()):
            * if first.1 == last.1:
              * new_line = last.0.truncate(-1) + first.0
              * segments.push((new_line,first.1))
      * unique_segment_ids = list
      * for segment in segments:
        * look for the segment in segment_cache and retrieve an id (probably look based on end-point of segment, if there are multiple, we will have to do a comparison, but probably reversed)
        * if no id was found, then push the segment into the segment_cache, which should return the id.
        * unique_segment_ids.push(id,reversed?)
      * rings.push(unique_segment_ids)
    * polygons.push(rings)
  * polygon_segments[polygon.id] = polygons
* bezerify_segments = map
* for segment in segment_cache:
  * bezierify the segment and put into bezerified_segments with same cache id
* for each multi_polygon in polygon_segments:
  * new_multi_polygon_geometry = multi-polygon geometry
  * for each polygon in multi_polygon
    * new_polygon_geometry = polygon geometry:
    * for each ring in polygon:
      * new_ring_geometry = line geometry
      * for each (segment_id,reversed) in ring:
        * get the bezierified segment off of segment_cache
        * if reversed, then reverse it.
        * add segment to the new_ring_geometry
      * add new_ring_geometry to new_polygon_geometry
    * add new_polygon_geometry to multi_polygon_geometry
  * layer[polygon.id].geometry = new_multi_polygon_geometry
  * intersect the layer with the polygons in coastline -- TODO: Or not? If I can place an ocean layer *over* the items, then it will cover it up. A user can do the coastline intersection on their own if they really want it.
  * update polygon in layer
* save layer
